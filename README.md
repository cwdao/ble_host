...
- ✅ **双帧模式支持**:
  ...
  - **方向估计（DF）模式**: 支持单通道功率数据，显示RMS幅值或功率P
  - 自动识别帧类型，切换模式时自动清空状态
- ✅ **数据处理**: 
  - 频率计算（基于FFT，15秒数据窗口）
  - 统计分析（均值、最大值、最小值、标准差）
  - 实时呼吸估计（可配置更新间隔，支持CS模式）
- ✅ **数据保存与加载**:
  - 支持保存所有帧或最近N帧数据
  - 自动保存功能（可配置路径）
  - 加载保存的文件进行离线分析
...
- **保存**：保存时会记录帧类型（DF/CS），文件名自动添加前缀（DF_/CS_）
- **加载**：加载文件时自动识别帧类型，自动设置相应模式
...
### 文件加载问题
- 确认文件格式正确（保存的文件为JSON格式）
- 检查文件版本是否兼容
- 查看日志窗口的错误信息
- **DF文件**：确认文件包含 `frame_type: "direction_estimation"`
